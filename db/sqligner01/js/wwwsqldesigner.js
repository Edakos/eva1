...
SQL.Visual.prototype.execSQL = function(sql, sql_onerror) {
    if (!SQL.Designer.ignoreDB) {
        if (!sql) { return; }
        /*
        if (typeof(sql_onerror) != 'undefined') {
            sql.prototype.onerror = sql_onerror;
            alert(sql.prototype.onerror);
        }
        */
        SQL.Designer.queries.push(sql);

        if (SQL.Designer.queries.length == 1) {
            this._toDB(sql, 0);
        }
    }
}

SQL.Visual.prototype._toDB = function(sql, count) {
    if (typeof(count) == 'undefined') {count = 0;}
    sql = sql.replace(/¨/g,'"');
    //alert(sql);
    //prompt('SQL', sql);
    if (!sql) { return; }
    var bp = SQL.Designer.getOption("xhrpath");
    var url = bp + "backend/php-postgresql/?action=execute&count=" + count + "&sql=" + encodeURIComponent(sql);
    //prompt('URL', url);
    OZ.Request(url, this._fromDB, {method:"get", xml:false});
}

SQL.Visual.prototype._fromDB = function(data, code) {
    //alert(code + ': ' + data);
    data = data.split('|||');
    ...
    if (code != 201 && count < 3) {
        window.setTimeout('SQL.Designer._toDB("' + sql.replace(/"/g,"¨") + '", ' + count + ')', 1000);
    /*
    } else if (typeof(SQL.Designer.queries[0].prototype.onerror) != 'undefined' && SQL.Designer.queries[0].prototype.onerror != '') {
        SQL.Designer.queries[0] = SQL.Designer.queries[0].prototype.onerror;
        SQL.Designer.queries[0].prototype.onerror = '';
        SQL.Designer._toDB(SQL.Designer.queries[0]);
    */    
    } else {
        //prompt('Error executing SQL query: \n\n' + error, sql);
        SQL.Designer.queries.shift();
        
        if (SQL.Designer.queries.length > 0) {
            //alert('Se ejecuta desde la cola de ' +  SQL.Designer.queries.length + ' + 1: ' + sql);
            SQL.Designer._toDB(SQL.Designer.queries[0], 0);
        }
    }
}
...
//alert("Visual.prototype.setTitle(" + text + ")");
...
SQL.Row.prototype.select = function() {
    ...
    this.selected = true;
    this.redraw(); //alert('redraw desde Row.select');
}

SQL.Row.prototype.deselect = function() {
    if (!this.selected) { return; }
    this.selected = false;
    this.redraw(); //alert('redraw desde Row.deselect');
    this.collapse();
}
...
SQL.Row.prototype.setTitle = function(t) {
    ...
    if (this.data.title == '') {
        ...
        // crear si no es ID
        if (t != 'id') { 
            elm = this.getDataType();
            column = '"' + t + '" ' + elm.getAttribute("sql");
            
            if (elm.getAttribute("length") == "1" && this.data.size) { column += "(" + this.data.size + ")"; }
            ...
            if (this.data.def || this.data.def === null) {
                ...
                if (d === null) { 
                    d = "NULL"; 
                } else if (d != "CURRENT_TIMESTAMP" && d != "now()") { 
                    d = q + d + q; 
                }
                column += " DEFAULT " + d;
            }
            ...
            this.execSQL('ALTER TABLE "' + this.owner.data.title + '" ADD COLUMN ' + column);
            
            if (this.data.comment !== null && this.data.comment != '') {
                this.execSQL('COMMENT ON COLUMN "' + this.owner.data.title + '"."' + column + '" IS ' + "'" + this.data.comment + "'");
            }
        }
    } else if (this.data.title != t) {
        // actualizar
        this.execSQL('ALTER TABLE "' + this.owner.data.title + '" RENAME COLUMN "' + this.data.title + '" TO "' + t + '"');
        //msg = ''; for (x in this.data) { msg += x + ': ' + this.data[x] + '\n'; } alert(msg);
    } else {
        //sin cambios
        return;
    }
...
SQL.Row.prototype.update = function(data) { /* update subset of row data */
    for (x in this.data) {
        if (typeof(data[x]) == 'undefined') { data[x] = this.data[x]; }    
    }
    if (data.nll && data.def && data.def.match(/^null$/i)) { data.def = null; }
    if (!data.nll && data.def === null) { data.def = ""; }
    ...
    if (this.data.title != '') {
        ...
        // solo si el campo no es nuevo:

        //msg = ''; for (x in data) { msg += x + ': ' + data[x] + '\n'; } alert(msg);        
        if (    this.data.nll != data.nll 
            ||  this.data.def != data.def
        ) {
            this.data.def = data.def;
            this.data.nll = data.nll;
            var elm = this.getDataType();

            sql = 'ALTER TABLE "' + this.owner.data.title + '" ALTER COLUMN "' + this.data.title + '"';
            ...
            if (this.data.def || this.data.def === null) {
                ...
                this.execSQL(sql + " SET DEFAULT " + d);
            }
            
            if (this.data.nll) {
                this.execSQL(sql + " DROP NOT NULL");
            } else {
                this.execSQL(sql + " SET NOT NULL");
            }
        }
        ...
        if (
            ...
            if (elm.getAttribute("length") == "1" && this.data.size) { type += "(" + this.data.size + ")"; }
            
            this.execSQL('ALTER TABLE "' + this.owner.data.title + '" DROP COLUMN "' + this.data.title + '" CASCADE');
            
            
            column = '"' + this.data.title + '" ' + elm.getAttribute("sql");
...
SQL.Row.prototype.changeComment = function(e) {
    var comment = prompt(_("commenttext"),this.data.comment);
    comment = comment || "";
    ...
    if (this.data.comment != comment) {
        ...
        this.dom.comment.innerHTML = this.data.comment;
        this.execSQL('COMMENT ON COLUMN "' + this.owner.data.title + '"."' + this.data.title + '" IS ' + "'" + this.data.comment + "'");
    }
}
...
SQL.Row.prototype.addRelation = function(r) {
    ...
    //this.relations2.push(r);
    //alert('campo ' + this.getTitle() + ': ' + this.relations.length + ': ' + this.relations2.length);
}
...
SQL.Row.prototype.destroy = function() {
    ...
    this.execSQL('ALTER TABLE "' + this.owner.data.title + '" DROP COLUMN "' + this.data.title + '"');
...
SQL.Relation.prototype.init = function(owner, row1, row2) {
    ...
    this.execSQL('ALTER TABLE "' + row2.owner.data.title + '" ADD CONSTRAINT "fk_' + row2.owner.data.title + '_' + row1.owner.data.title + '" FOREIGN KEY ("' + row2.data.title + '") REFERENCES "' + row1.owner.data.title + '"("id")  MATCH SIMPLE ON UPDATE CASCADE ON DELETE NO ACTION;');
...
SQL.Relation.prototype.destroy = function() {
    ...
    this.execSQL('ALTER TABLE "' + this.row2.owner.data.title + '" DROP CONSTRAINT "fk_' + this.row2.owner.data.title + '_' + this.row1.owner.data.title + '"');
...
SQL.Table.prototype.setTitle = function(t) {
    if (this.data.title == '') {
        //crear
        t = ("new_table_" + (new Date()).valueOf() + "_" + Math.floor(Math.random()*1001)); 
        //this.execSQL('CREATE TABLE "' + t + '" ("id" SERIAL, CONSTRAINT "pk_' + t + '" PRIMARY KEY ("id"))');
        this.execSQL('CREATE SEQUENCE ' + t + '_id_seq');
        this.execSQL('CREATE TABLE "' + t + '" ("id" INTEGER DEFAULT NEXTVAL(' + "'" + t + "_id_seq'" + '::regclass), CONSTRAINT "pk_' + t + '" PRIMARY KEY ("id"))');
    } else if (this.data.title != t) {
        //actualizar
        this.execSQL('ALTER TABLE "' + this.data.title + '" RENAME TO "' + t + '"');
        //this.execSQL('ALTER SEQUENCE "' + this.data.title + '_id_seq" RENAME TO "' + t + '_id_seq"'
        
        this.execSQL('CREATE SEQUENCE ' + t + '_id_seq');
        this.execSQL("SELECT SETVAL ('" + t + "_id_seq'::regclass, (select nextval('" + this.data.title + "_id_seq'::regclass)))");
        this.execSQL("ALTER TABLE " + t + " ALTER COLUMN id DROP DEFAULT");
        this.execSQL("ALTER TABLE " + t + " ALTER COLUMN id SET DEFAULT nextval('" + t + "_id_seq'::regclass)");
        this.execSQL('DROP SEQUENCE ' + this.data.title + '_id_seq');
        
        //alert(this.data.title + ' != ' + t);
        this._updateFK(t);
    } else {
        //sin cambios
        return;
...
SQL.Table.prototype._updateFK = function(word) {
    if (!SQL.Designer.ignoreDB) {
        if (typeof(word) == 'undefined') { word = this.getTitle(); }
        
        var bp = SQL.Designer.getOption("xhrpath");
        var url = bp + "backend/php-postgresql/?action=singularize&word=" + encodeURIComponent(word);
        //prompt('URL', url);
        
        OZ.Request(url, this.bind(this._singularizeresponse), {method:"get", xml:false});
    }
}

SQL.Table.prototype._singularizeresponse = function(data, code) {    
    if (code == 201 && data != null && data != '') {
        new_title = data + '_id';
        //msg = ''; relations = this.getRelations();for (x in relations) { msg += x + ': ' + relations[x] + '\n'; } alert('relations: \n\n' + msg);
        //alert("this.keys.length: " + this.keys.length)
        for (var i = 0; i < this.keys.length; i++) {
            key = this.keys[i];
            //alert("key.rows.length: " + key.rows.length)
            for (var j = 0; j < key.rows.length; j++) {
                row = key.rows[j];
                //alert(row.getTitle() + ".relations.length: " + row.relations.length);
                //msg = ''; for (x in row) { msg += x + ': ' + row[x] + '\n'; } alert('en row: ' + msg);
                for (var k = 0; k < row.relations.length; k++) {
                    var relation = row.relations[k];
                    //msg = ''; for (x in relation) { msg += x + ': ' + relation[x] + '\n'; } alert('DOS: ' + msg);
                    old_title = relation.row2.getTitle();
                    if (new_title != old_title) { // si existe cambio:   
                        var r = old_title.match(/_id$/i);
                        
                        if (r) { //la clave foranea de la tabla foranea termina en '_id':
                            //alert('CAMBIANDO TITULO DE COLUMNA de ' + old_title + ' A: ' + new_title);
                            relation.row2.setTitle(new_title);
                        }
                    }
                }
            }
...
SQL.Table.prototype.addCreated = function() {
    title = 'created';
    data = {type:18, def:'CURRENT_TIMESTAMP', nll:false};
    var r = this.addRow(title, data);
    return r;
}

SQL.Table.prototype.addModified = function() {
    title = 'modified';
    data = {type:18, def:'CURRENT_TIMESTAMP', nll:false};
    var r = this.addRow(title, data);
    return r;
...
SQL.Table.prototype.redraw = function() {
    ...
    //Autosave:
    if (SQL.Designer.io) {SQL.Designer.io.serverautosave(); }
}
...
SQL.Table.prototype.setComment = function(c) {
    c = (c === null ? '' : c);
    if (this.data.comment != c) {
        this.data.comment = c;
        this.execSQL('COMMENT ON TABLE "' + this.data.title + '" IS ' + "'" + this.data.comment + "'");
    }
}
...
SQL.Table.prototype.destroy = function() {
    ...
    for (var i = 0; i < this.keys.length; i++) {
        ...
        for (var j = 0; j < key.rows.length; j++) {
            ...
            for (var k = 0; k < relations.length; k++) { 
                ...
                if (relation.row1.data.title == row.data.title) {
                    relation.row2.destroy();
                }
            }
        }
    ...
    this.execSQL('DROP TABLE "' + this.data.title + '"');
...
SQL.IO.prototype.serverautosave = function(e) {
    if (AUTOSAVE == 'NO') {this.delayAutoSave(false);}
    if (AUTOSAVE != 'YES') {return;}
    var name = this._name;
...
SQL.IO.prototype.delayAutoSave = function(isByTimeout) {
    if (!isByTimeout) {
        AUTOSAVE = 'WAIT';
        window.setTimeout('SQL.Designer.io.delayAutoSave(true)', CONFIG.AUTOSAVE_WAIT_TIME * 1000);
    } else {
        AUTOSAVE = 'YES';
        this.serverautosave();
    }
...
SQL.TableManager.prototype.init = function(owner) {
    ...
    var ids = ["addtable","removetable","addrow","tablekeys"];
    for (var i=0;i<ids.length;i++) {
        var id = ids[i];
        var elm = OZ.$(id);
        this.dom[id] = elm;
...
SQL.TableManager.prototype.addRow = function(e) {
    var newrow = this.selected.addRow("new_field_" + (new Date()).valueOf() + "_" + Math.floor(Math.random()*1001));
...
SQL.TableManager.prototype.select = function(table) { /* activate table */
    ...
    if (this.selected) {
        ...
        //Adding rows "created" and "modified", if not exists:
        var exists_created = false;
        var exists_modified = false;

        for (var i = 0; i < this.selected.rows.length; i++) {
            var title = this.selected.rows[i].getTitle();
            exists_created = (!exists_created) ? title == 'created' : true;
            exists_modified = (!exists_modified) ? title == 'modified' : true;
        }

        msg = '';

        if (!exists_created) { 
            this.selected.addCreated();
            msg += '\n\n- Added "created" field.';
        }
        if (!exists_modified) { 
            this.selected.addModified(); 
            msg += '\n\n- Added "modified" field.';
        }

        //Changing primary keys datatypes to serial:
        
        for (var i = 0; i < this.selected.keys.length; i++) {
            key = this.selected.keys[i];
            for (var j = 0; j < key.rows.length; j++) {
                row = key.rows[j];
                update = false;
                if (row.data.type != 4) {
                    msg += '\n\n- Change datatype of primary key "' + row.getTitle() + '" to SERIAL';
                    update = true;
                }
                if (row.data.size != '') {
                    msg += '\n\n- Change size of primary key "' + row.getTitle() + '" to none.';
                    update = true;
                }
                if (row.data.def != '') {
                    msg += '\n\n- Change default value of primary key "' + row.getTitle() + '" to none.';
                    update = true;
                }
                if (row.data.nll) {
                    msg += '\n\n- Change null of primary key "' + row.getTitle() + '" to false.';
                    update = true;
                }
                if (!row.data.ai) {
                    msg += '\n\n- Change auto-numeric of primary key "' + row.getTitle() + '" to true.';
                    update = true;
                }
                if (update) {
                    row.update({
        ...
        //Changing foreign keys datatypes to integer:
        
        for (var i = 0; i < this.selected.rows.length; i++) {
            var row = this.selected.rows[i];
            for (var j = 0; j < row.relations.length; j++) {
                var relation = row.relations[j];
                if (row.getTitle() == relation.row2.getTitle()) { //foreign keys
                    //alert (row.getTitle() + ': ' + relation.row1.owner.getTitle() + '.' + relation.row1.getTitle() + ' -> ' + relation.row2.owner.getTitle() + '.' + relation.row2.getTitle());
                    update = false;
                    if (row.data.type != 0) {
                        msg += '\n\n- Change datatype of foreign key "' + row.getTitle() + '" to INTEGER.';
                        update = true;
                    }
                    if (row.data.size != '') {
                        msg += '\n\n- Change size of foreign key "' + row.getTitle() + '" to none.';
                        update = true;
                    }
                    if (row.data.def != '') {
                        msg += '\n\n- Change default value of foreign key "' + row.getTitle() + '" to none.';
                        update = true;
                    }
                    if (!row.data.nll) {
                        msg += '\n\n- Change null of foreign key "' + row.getTitle() + '" to true.';
                        update = true;
                    }
                    if (row.data.ai) {
                        msg += '\n\n- Change auto-numeric of foreign key "' + row.getTitle() + '" to false.';
                        update = true;
                    }
                    if (update) {
                        row.update({
...
SQL.TableManager.prototype.click = function(e) { /* finish adding new table */
    ...
    if (this.adding) {
        ...
        newtable = this.owner.addTable('',x,y);
        var r = newtable.addRow("id",{ai:true, type:4, def:'', nll:false});
        var k = newtable.addKey("PRIMARY","");
        k.addRow(r);

        //Adding CakePHP fields:
        newtable.addRow("name",{type:10, size:1000, def:''});
        newtable.addRow("notes",{type:11, def:''});
        newtable.addCreated();
        newtable.addModified();
    }
...
SQL.RowManager.prototype.init = function(owner) {
    ...
    this.selected = null;
    this.creating = false;
//alert('this.creating desde RowManager.prototype.init: ' + this.creating);
    this.connecting = false;

    this.managing = false;
    
    //var ids = ["editrow","removerow","uprow","downrow","foreigncreate","foreignconnect","foreigndisconnect"];
    var ids = ["removerow","uprow","downrow","foreigncreate"];
    for (var i=0;i<ids.length;i++) {
        var id = ids[i];
        var elm = OZ.$(id);
        this.dom[id] = elm;
...
SQL.RowManager.prototype.select = function(row) { /* activate a row */
    ...
    this.redraw(); //alert('redraw desde select');
}

SQL.RowManager.prototype.tableClick = function(e) { /* create relation after clicking target table */
//  alert("XXX this.creating desde RowManager.tableClick: " + this.creating);
    if (!this.creating) { return; }
    var row1 = this.selected;
    var table2 = e.target;
    
    var pattern = this.owner.getOption("pattern");
    pattern = pattern.replace(/%T/g,row1.owner.getTitle());
    pattern = pattern.replace(/%t/g,table2.getTitle());
    pattern = pattern.replace(/%R/g,row1.getTitle());
    //aqui
    
    row2_data = [];
    //for (x in r1.data) { r2_data[x] = r1.data[x]; }
    ...
    //msg = ''; for (x in r1.data) { msg += x + ': ' + r1.data[x] + '\n'; } alert(msg);
    //msg = ''; for (x in r2_data) { msg += x + ': ' + r2_data[x] + '\n'; } alert(msg);
    
    var row2 = table2.addRow(pattern, row2_data);
    //r2.update({"ai":false, type:0});
    this.owner.addRelation(row1, row2);
    
    row1.owner._updateFK();
}
...
/*
    this.endConnect();
    if (this.creating) {
        this.endCreate(); //alert('endCreate desde ');
    } else {
        this.creating = true;
        this.dom.foreigncreate.value = "["+_("foreignpending")+"]";
    }
*/
    //join foreigncreate and foreignconnect:
    if (this.creating) {
        this.endCreate(); //alert('endCreate desde foreigncreate');
    } else {
        this.creating = true;
        this.dom.foreigncreate.value = "["+_("foreignpending")+"]";
    }
//alert('this.creating desde RowManager.prototype.foreigncreate: ' + this.creating);
}

SQL.RowManager.prototype.foreignconnect = function(e) { /* start drawing fk */
    this.endCreate(); //alert('endCreate desde foreignconnect');
    if (this.connecting) {
        this.endConnect();
    } else {
...
SQL.RowManager.prototype.foreigndisconnect = function(e) { /* remove connector */
    ...
    this.redraw(); //alert('redraw desde foreigndisconnect');
}

SQL.RowManager.prototype.endCreate = function() {
    this.creating = false;
//alert('this.creating desde RowManager.prototype.endCreate: ' + this.creating);
...
SQL.RowManager.prototype.up = function(e) {
    this.selected.up();
    this.redraw(); //alert('redraw desde up');
}

SQL.RowManager.prototype.down = function(e) {
    this.selected.down();
    this.redraw(); //alert('redraw desde down');
}
...
SQL.RowManager.prototype.redraw = function() {
    this.endCreate(); //alert('endCreate desde redraw');
    //this.endConnect();
    if (this.selected) {
...
SQL.Designer.prototype.addTable = function(name, x, y) {
    ...
    t.redraw(); //alert('redraw desde Designer.addTable');
